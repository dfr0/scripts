...
log="execute_command_noroot_per_host.log"

###AYUDA####

display_usage() {
        echo "El uso del script es: "
        echo -e "./script \"host host1 host2\" comando\n"
	echo
        }
...
for i in $host; do echo "####$i  $timestamp####";sshpass -p $PWD ssh -t -o ConnectTimeout=5 $USER@$i "$execute" ;done |  tee -a $log
}
